describe "main" do
  before(:all) do
    ...
    # Will be located at: /tmp/terraspace/test-harnesses/elastic_beanstalk-harness
    terraspace.build_test_harness(
      name:    "elastic_beanstalk-harness",
      modules: "#{ts_root}/app/modules", # include all modules in folder
      stacks:  {elastic_beanstalk: stack_path},
      # override demo stack tfvars for testing
      config:  "#{stack_path}/test/spec/fixtures/config",
      tfvars:  {elastic_beanstalk: "#{stack_path}/test/spec/fixtures/tfvars/test.tfvars"},
    )
    terraspace.up("elastic_beanstalk")
  end
  after(:all) do
    terraspace.down("elastic_beanstalk")
  end
  ...
  it "successful deploy" do
    ...
    output_value = terraspace.output("elastic_beanstalk", "bucket_name")
